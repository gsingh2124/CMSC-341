...
private:
    ...
    Node *getLastNode(Node *aNode);//gets last node in list, used internally for other functions
    int getGraphSize();//returns the current size of the list (m_next based)
    void insertAtEnd(Node * aNode);//inserts node at the end of the list
    void testPrintGraph();
};